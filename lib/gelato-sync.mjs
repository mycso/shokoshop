...
const blobInflight = new Map(); // sourceKey → in-progress persist promise, so concurrent callers share one download+upload

/**
 * Runs `fn` over `items` with at most `concurrency` calls in flight at once.
 * Plain product/variant fetches are independent network round trips, so
 * running them fully sequentially (as this used to) turns a store with a
 * couple hundred variants into a multi-minute wait; bounding concurrency
 * instead of going fully parallel keeps us from hammering Gelato's API.
 */
 ...
 */
// Set once a write comes back "suspended" (a Blob store billing/plan state,
// not a transient error) so the rest of this run stops paying for a
// download-then-doomed-upload on every remaining image and just falls back
// to the original Gelato-hosted URL instead.
let blobWritesSuspended = false;

function isSuspendedError(err) {
  return err instanceof Error && /suspended/i.test(err.message);
}
...
export async function persistImageToBlob(url) {
  ...
  if (blobInflight.has(sourceKey)) return blobInflight.get(sourceKey);
  if (blobWritesSuspended) return url;
  ...
  const promise = (async () => {
    ...
    try {
      ...
      if (isSuspendedError(err)) blobWritesSuspended = true;
      throw err;
    }
  ...
  blobInflight.set(sourceKey, promise);
  try {
    return await promise;
  } finally {
    blobInflight.delete(sourceKey);
  }
}
...
export async function fetchAllProductsWithPricesAndImages({ apiKey, storeId, onLog = () => {} }) {
  ...
  // Two independent concurrency pools (not one shared limiter): a product
  // task stays "active" in the product pool for as long as its variant
  // fetches take, so reusing a single limiter for both levels would let
  // fully-occupied product slots starve the variant tasks they're waiting
  // on — a deadlock. Separate pools avoid that.
  const results = await mapWithConcurrency(products, 4, async (product) => {
    ...
    const variantPrices = {};
    const variantImages = {};
    await mapWithConcurrency(variants, 8, async (v) => {
      let vdata;
      try {
  ...
  for (const img of detail.productImages ?? []) {
    ...
    const putRes = await fetch(`${base}/products/${productId}/images/${img.id}`, {
      ...
      body: JSON.stringify({
        ...
        isPrimary: false,
        skipPublishing: true, // don't re-trigger the product webhook from this programmatic update
