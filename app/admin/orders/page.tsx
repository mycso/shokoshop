    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <form action="/admin/orders" method="GET" className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          name="status"
          defaultValue={status ?? ""}
          className="rounded-xl border border-gray-200 bg-white text-sm px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-brand dark:bg-gray-800 dark:border-gray-700 dark:text-white"
        >
        ...
        {(q || status) && (
          <Link
            href="/admin/orders"
            className="text-sm font-medium px-4 py-2.5 rounded-xl border border-gray-200 text-gray-600 hover:border-brand transition-colors text-center dark:border-gray-700 dark:text-gray-200"
          >
            Clear
          </Link>
        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">
                    Order
                  </th>
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell dark:text-gray-400">
                    Customer
                  </th>
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell dark:text-gray-400">
                    Date
                  </th>
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider dark:text-gray-400">
                    Status
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {paged.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-6 py-4 text-gray-500 hidden md:table-cell dark:text-gray-400">
                    ...
                    <td className="px-6 py-4">
                      <Link
                        ...
                        className="text-gray-400 hover:text-brand transition-colors dark:text-gray-500"
                      >
              ...
              <div className="flex gap-2">
                {page > 1 ? (
                  <Link
                    ...
                    className="text-sm font-medium px-4 py-2 rounded-xl border border-gray-200 text-gray-700 hover:border-brand transition-colors dark:border-gray-700 dark:text-gray-200"
                  >
                    Previous
                  </Link>
                ) : (
                  <span className="text-sm font-medium px-4 py-2 rounded-xl border border-gray-100 text-gray-300 dark:border-gray-800 dark:text-gray-600">
                    Previous
                  </span>
                )}
                {page < totalPages ? (
